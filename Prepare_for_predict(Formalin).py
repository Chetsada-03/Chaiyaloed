...
main_dir = os.path.join("newmodel", "Formalin") #main folder ***
PREDICT = os.path.join("newmodel", "newupload")
...
def machine_learning(X, y, key):
    ...
    for model_name, model in models.items(): # Iterate over the models
        ...
        with open(f"{model_name}_model.pkl", "wb") as f: # Open a file to store the best model
...
# Process for predict
main_dir = os.path.join(".", "static") # main folder ***
predict_dirs = ["PREDICT_OUTPUT_CROP_IMAGE", "PREDICT_OUTPUT_REAL_MARK", 
                "PREDICT_OUTPUT_CROP_MARKED", "PREDICT_OUTPUT_CROP_WARP", 
                "PREDICT_OUTPUT_WARP_ADD", "PREDICT_ANAL_REAL", 
                "PREDICT_ANAL_WARP", "PREDICT_ANAL_REAL_WARP_ADD"]
for folder in predict_dirs:
    predict_dirs_path = os.path.join(main_dir, folder)
    os.makedirs(predict_dirs_path, exist_ok=True)

PREDICT_OUTPUT_CROP_IMAGE = os.path.join(main_dir, "PREDICT_OUTPUT_CROP_IMAGE")
PREDICT_OUTPUT_REAL_MARK = os.path.join(main_dir, "PREDICT_OUTPUT_REAL_MARK")
PREDICT_OUTPUT_CROP_MARKED = os.path.join(main_dir, "PREDICT_OUTPUT_CROP_MARKED")
PREDICT_OUTPUT_CROP_WARP = os.path.join(main_dir, "PREDICT_OUTPUT_CROP_WARP")
PREDICT_OUTPUT_WARP_ADD = os.path.join(main_dir, "PREDICT_OUTPUT_WARP_ADD")
PREDICT_ANAL_REAL = os.path.join(main_dir, "PREDICT_ANAL_REAL")
PREDICT_ANAL_WARP = os.path.join(main_dir, "PREDICT_ANAL_WARP")
PREDICT_OUTPUT_ANAL_WARP_ADD = os.path.join(main_dir, "PREDICT_ANAL_REAL_WARP_ADD")

#Delete all image in last running code
directories = [PREDICT_OUTPUT_CROP_IMAGE, PREDICT_OUTPUT_REAL_MARK, PREDICT_OUTPUT_CROP_MARKED, PREDICT_OUTPUT_CROP_WARP, 
               PREDICT_OUTPUT_WARP_ADD, PREDICT_ANAL_REAL, PREDICT_ANAL_WARP, PREDICT_OUTPUT_ANAL_WARP_ADD] #Define subfolder to delete image inside
...
min_column_number_df = pd.DataFrame() 
Image_name_data_df_loop_predict = []  # For storing metadata of predict images
Air_data_df_predict = pd.DataFrame()  # For storing metadata of Air predict images
result_data_predict = pd.DataFrame()  # For processed predict data
result_anal_datafull_predict = pd.DataFrame()  # For further analysis of predict data

model_path = os.path.join(os.getcwd(), "newmodel", "Formalin")
output_path = os.path.join(model_path, 'Data_ready_predict.pkl')

# Process images from predict directory
for filename in os.listdir(PREDICT):
    ...
    try:
        ...
        cv2.drawContours(marked_crop_img, [reordered_corners], 0, (0, 255, 0), 2) # Draw the reordered corners on the marked cropped image
        output_path_marked1 = os.path.join(PREDICT_OUTPUT_CROP_MARKED, f"marked_{filename}") # Construct the output path for the marked cropped image
        ...
        predict_output_path_warp = os.path.join(PREDICT_OUTPUT_CROP_WARP, f"warp_{filename}") # Construct the output path for the warped image
        #cv2.imwrite(predict_output_path_warp, warped_Air_image) # Commented out code for saving the warped image
        ...
        predict_output_path_marked = os.path.join(PREDICT_OUTPUT_REAL_MARK, f"marked_{filename}") # Construct the output path for the marked image
        cv2.imwrite(predict_output_path_marked, img_real_for_center) # Save the marked image with the new center and reordered corners
        Warp_add_Air_image = condition(warped_Air_image) # Apply the condition function to the warped image
        
        predict_output_path_warp_add = os.path.join(PREDICT_OUTPUT_WARP_ADD, f"warp_{filename}") # Construct the output path for the conditioned warped image
        cv2.imwrite(predict_output_path_warp_add, Warp_add_Air_image) # Save the conditioned warped image
        ...
        Image_name_data = {'Light_Source': [Light_Source], 'Solute_1': [solute_1],'Solute_2': [solute_2], 'Solvent': [solvent], 'Picture_Number': [Picture_number], 'Concentration_1_ppm': [Concentration_1_ppm],'Concentration_2_ppm': [Concentration_2_ppm] } # Create a dictionary with image name data
        Image_name_data_df = pd.DataFrame(Image_name_data) # Convert the image name data dictionary to a DataFrame
        Image_name_data_df_loop_predict.append(Image_name_data_df) # Append the image name data DataFrame to the loop list
        img_air_df = Average_lightness(Warp_add_Air_image, Image_name_data)
        Air_data_df_predict = pd.concat([Air_data_df_predict,img_air_df])
        ...
        new_Anal_Position = np.intp(CenterToFiveParamFormRef(new_Anal_center, new_reordered_corners, angle, height_solution_ratio, width_solution_ratio, more_red)) # Calculate the new analysis position using the CenterToFiveParamFormRef function
        ...
        output_path_marked_anal = os.path.join(PREDICT_ANAL_REAL, f"marked_{filename}") # Construct the output path for the marked analysis image
        ...
        predict_output_path_warp_anal = os.path.join(PREDICT_ANAL_WARP, f"warp_{filename}") # Construct the output path for the warped analysis image
        cv2.imwrite(predict_output_path_warp_anal, warped_image_anal) # Save the warped analysis image
        Local_minimum_each_image_Df, min_column_number_df,median_minimum = local_minimum_find(warped_image_anal, image_name, Local_minimum_each_image_Df, min_column_number_df) # Find the local minimum using the local_minimum_find function
        image_data_predict = lightness_adjust(Warp_add_Air_image, image_name)
        #image_data_predict = lightness_adjust(Full_image, image_name)# Adjust the lightness of the full image using the lightness_adjust function
        result_data_predict = pd.concat([result_data_predict, image_data_predict], ignore_index=True) # Concatenate the image data to the result_data DataFrame
        #print(f"Processed, marked, mark real, and warped: {predict_output_path_marked}, {PREDICT_OUTPUT_REAL_MARK} ,{predict_output_path_warp}") # Print a message indicating successful processing, marking, and warping
...
Image_name_data_df_full_predict = pd.concat(Image_name_data_df_loop_predict, ignore_index=True)  # Concatenate all DataFrames in Image_name_data_df_loop into a single DataFrame
image_name_df_predict = result_data_predict.iloc[:, [0]]  # Extract first column (Image Names)
result_data_noname_predict = result_data_predict.drop(result_data_predict.columns[0], axis=1)
result_data_noname_average_predict = result_data_noname_predict.mean(axis=1)
with open(f'{model_path}\\median_data.pkl', 'rb') as f:
    median_data = pickle.load(f)

adjust_data_predict = median_data - result_data_noname_average_predict
adjust_data_array_predict = adjust_data_predict.values
reshaped_adjust_data_predict = adjust_data_array_predict[:, np.newaxis]

Air_data_predict = Air_data_df_predict.drop(columns=["Image_Name"]).values
New_Air_data_predict = Air_data_predict + reshaped_adjust_data_predict
New_Air_data_df_predict = pd.DataFrame(New_Air_data_predict)
New_Air_data_df_predict = pd.concat([Image_name_data_df_full_predict, New_Air_data_df_predict], axis=1)
        
for filename_predict_warp_img in os.listdir(PREDICT_ANAL_WARP):  # Iterate over filenames in the OUTPUT_ANAL_WARP directory
    img_path = os.path.join(PREDICT_ANAL_WARP, filename_predict_warp_img)  # Construct the full image path
    ...
    image_namefull_anal_predict = os.path.splitext(filename)[0]  # Extract the image name without extension
    image_datafull_anal_predict = Average_lightness(warped_image_anal_add, image_namefull_anal_predict)  # Call the Average_lightness function
    result_anal_datafull_predict = pd.concat([result_anal_datafull_predict, image_datafull_anal_predict], ignore_index=True)  # Concatenate image_datafull_anal to result_anal_datafull
    ...
    #print(filename_predict_warp_img)  # Print the current filename
    predict_output_path_warp_adjust = os.path.join(PREDICT_OUTPUT_ANAL_WARP_ADD, f"marked_{filename_predict_warp_img}")  # Construct the output path for the marked image
    cv2.imwrite(predict_output_path_warp_adjust, warp_img_crop_add)  # Save the marked image

# Additional Analysis Data 
result_data_full_noname_anal_predict = result_anal_datafull_predict.drop(result_data_predict.columns[0], axis=1)
result_data_full_noname_anal_predict = result_data_full_noname_anal_predict.values
new_result_data_anal_predict = result_data_full_noname_anal_predict + reshaped_adjust_data_predict
new_result_data_anal_df_predict = pd.DataFrame(new_result_data_anal_predict)
Crude_result_df_predict = new_result_data_anal_df_predict
Crude_result_df_predict = new_result_data_anal_df_predict.astype(float)
What_happen1_predict = Crude_result_df_predict.insert(0, "Image_Name", image_name_df_predict)

# Save PREDICT DataFrame to Excel
predict_output_file = os.path.join(main_dir, "predict_processed.xlsx")
Crude_result_df_predict.to_excel(predict_output_file, sheet_name="Sheet1", index=False)

DATA_FOR_not_done_predict = Crude_result_df_predict  # Assign Crude_result_df to DATA_FOR_not_done
DATA_FOR_not_done_predict.columns = image_datafull_anal_predict.columns.values  # Update column names of DATA_FOR_not_done
DATA_FOR_not_done_predict = DATA_FOR_not_done_predict.drop(DATA_FOR_not_done_predict.columns[0], axis = 1)  # Drop the first column from DATA_FOR_not_done
DATA_FOR_not_done_predict = pd.concat([Image_name_data_df_full_predict, DATA_FOR_not_done_predict], axis = 1)  # Concatenate Image_name_data_df_full with DATA_FOR_not_done

DATA_with_Solute_predict = DATA_FOR_not_done_predict[DATA_FOR_not_done_predict['Solute_1'] != "Blank"]  # Filter rows where 'Solute_1' is not "Blank"
with open(f'{model_path}\\DATA_BLANK_grouped_average.pkl', 'rb') as f:
    DATA_BLANK_grouped_average = pickle.load(f)

DATA_FOR_PREDICT = DATA_with_Solute_predict.copy()  # Create a copy of DATA_with_Solute and assign it to DATA_FOR_PREDICT
for index, row in DATA_BLANK_grouped_average.iterrows():  # Iterate over rows in DATA_BLANK_grouped_average
    mask = (DATA_with_Solute_predict['Light_Source'] == row['Light_Source']) & \
           (DATA_with_Solute_predict['Solvent'] == row['Solvent'])  # Create a mask based on 'Light_Source' and 'Solvent'
    for col in DATA_with_Solute_predict.filter(like='pixel').columns:  # Iterate over columns containing 'pixel'
        DATA_FOR_PREDICT.loc[mask, col] = row[col] - DATA_with_Solute_predict.loc[mask, col]  # Perform operations on DATA_FOR_PREDICT
DATA_FOR_PREDICT_READY = DATA_FOR_PREDICT.filter(like='pixel')
DATA_FOR_PREDICT_READY_df = pd.DataFrame(DATA_FOR_PREDICT_READY)
DATA_FOR_PREDICT_READY_df.columns = range(DATA_FOR_PREDICT_READY_df.shape[1])
with open(output_path, 'wb') as f:
    pickle.dump(DATA_FOR_PREDICT_READY_df, f)

columns_to_drop = ['Concentration_1_ppm', 'Concentration_2_ppm', 'Solute_1', 'Solute_2', 'Solvent', 'Light_Source', 'Picture_Number']
Air_data_df_predict = Air_data_df_predict.drop(columns = "Image_Name")
New_Air_data_df_predict = New_Air_data_df_predict.drop(columns = columns_to_drop)

models = {
    "RandomForest": f"{model_path}/RF_model.pkl",
    "SupportVectorRegression": f"{model_path}/SVR_model.pkl",
    "MLPRegressor": f"{model_path}/MLP_model.pkl",
    "MultiLinearRegression": f"{model_path}/MLR_model.pkl",
    "XGBRegressor": f"{model_path}/XGB_model.pkl"
}

tolerance = {
    "RandomForest": f"{model_path}/RF_MAPE.pkl",
    "SupportVectorRegression": f"{model_path}/SVR_MAPE.pkl",
    "MLPRegressor": f"{model_path}/MLP_MAPE.pkl",
    "MultiLinearRegression": f"{model_path}/MLR_MAPE.pkl",
    "XGBRegressor": f"{model_path}/XGB_MAPE.pkl"
}

selector_filename = os.path.join(model_path, "selector.pkl")
with open(selector_filename, "rb") as f:
    selector = pickle.load(f)

data_file = os.path.join(model_path, "Data_ready_predict.pkl")
if os.path.exists(data_file):
    with open(data_file, "rb") as f:
        DATA_FOR_PREDICT_READY_df = pickle.load(f)
else:
    print("❌ File not found Data_ready_predict.pkl")
    exit()

data_selected = selector.transform(DATA_FOR_PREDICT_READY_df)
...
for model_name, model_file in models.items():
    ...
    predicted_value = round(predictions.mean(), 4)
    
    mape_file = tolerance.get(model_name)
    if mape_file and os.path.exists(mape_file):
        try:
            with open(mape_file, "rb") as f:
                mape_value = pickle.load(f)
            if isinstance(mape_value, (int, float)): 
                mape_results[model_name] = round(mape_value, 4)
            else:
                print(f"❌ Value in {mape_file} Not a number (find: {type(mape_value)})")
        except Exception as e:
            print(f"❌ Unable to load MAPE values from {mape_file}: {e}")

    model_results.append((model_name, predicted_value, mape_results.get(model_name, "N/A")))

results_df = pd.DataFrame(model_results, columns=["Model", "Predicted Value (ppm)", "MAPE (%)"])

if results_df["MAPE (%)"].isnull().all():
    print("⚠️ There is no MAPE value available for download. Please check the MAPE file.")
else:
    best_model_row = results_df.loc[results_df["MAPE (%)"].idxmin()]
    best_model_name = best_model_row["Model"]
    best_predicted_value = best_model_row["Predicted Value (ppm)"]
    best_mape = best_model_row["MAPE (%)"]

    print(f"<b>Model:</b> {best_model_name}")
    print(f"<b>Predicted concentration:</b> {best_predicted_value} %w/v")
    print(f"<b>Mean absolute percentage error:</b> {best_mape} %")
